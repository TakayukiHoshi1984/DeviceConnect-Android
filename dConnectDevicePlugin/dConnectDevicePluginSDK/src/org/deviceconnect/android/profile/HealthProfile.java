...
public class HealthProfile extends DConnectProfile implements HealthProfileConstants {
    ...
    protected boolean onPutRequest(final Intent request, final Intent response) {
        ...
        if (ATTRIBUTE_HEART_RATE.equals(attribute)) {
            ...
            result = onPutOnHeartRate(request, response, serviceId, sessionKey);
        } else {
            MessageUtils.setUnknownAttributeError(response);
        }
    ...
    protected boolean onDeleteRequest(final Intent request, final Intent response) {
        ...
        if (ATTRIBUTE_HEART_RATE.equals(attribute)) {
            String serviceId = getServiceID(request);
            String sessionKey = getSessionKey(request);
            result = onDeleteOnHeartRate(request, response, serviceId, sessionKey);
        } else {
            MessageUtils.setUnknownAttributeError(response);
        }
    ...
    /**
     * onheartrateコールバック登録リクエストハンドラー.<br/>
     * onheartrateコールバックを登録し、その結果をレスポンスパラメータに格納する。
     * レスポンスパラメータの送信準備が出来た場合は返り値にtrueを指定する事。
     * 送信準備ができていない場合は、返り値にfalseを指定し、スレッドを立ち上げてそのスレッドで最終的にレスポンスパラメータの送信を行う事。
     * 
     * @param request リクエストパラメータ
    ...
    /**
     * onheartrateコールバック解除リクエストハンドラー.<br/>
     * onheartrateコールバックを解除し、その結果をレスポンスパラメータに格納する。
     * レスポンスパラメータの送信準備が出来た場合は返り値にtrueを指定する事。
     * 送信準備ができていない場合は、返り値にfalseを指定し、スレッドを立ち上げてそのスレッドで最終的にレスポンスパラメータの送信を行う事。
     * 
     * @param request リクエストパラメータ
